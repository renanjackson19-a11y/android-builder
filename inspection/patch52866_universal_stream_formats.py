...
old='''  int current=Math.max(0,Math.min(tvInlineSourceIndex,tvInlineSources.size()-1));String currentUrl=tvInlineSources.get(current);boolean disguisedTs=currentUrl!=null&&currentUrl.toLowerCase(java.util.Locale.US).contains(".m3u8")&&tvInlineTsFallbackIndex!=current;int target;if(disguisedTs){tvInlineTsFallbackIndex=current;target=current;}else{boolean canRetrySame=tvInlineRecoveryCount<2;target=canRetrySame?current:(current+1<tvInlineSources.size()?current+1:current);}tvInlineRecoveryCount++;final int generation=tvPlaybackGeneration;final int retryTarget=target;'''
new='''  int current=Math.max(0,Math.min(tvInlineSourceIndex,tvInlineSources.size()-1));String currentUrl=tvInlineSources.get(current);String path="";try{path=Uri.parse(currentUrl==null?"":currentUrl).getPath();}catch(Exception ignored){}String low=(path==null?"":path).toLowerCase(java.util.Locale.US);int target=current;boolean changedFormat=false;if(fromError){if((low.endsWith(".m3u8")||low.endsWith(".m3u"))&&tvInlineTsFallbackIndex!=current){tvInlineTsFallbackIndex=current;tvInlineHlsFallbackIndex=-1;changedFormat=true;}else if((low.endsWith(".ts")||low.isEmpty()||(!low.contains(".")&&!low.endsWith("/")))&&tvInlineHlsFallbackIndex!=current){tvInlineHlsFallbackIndex=current;tvInlineTsFallbackIndex=-1;changedFormat=true;}else if(tvInlineHlsFallbackIndex==current&&tvInlineTsFallbackIndex!=current){tvInlineTsFallbackIndex=current;tvInlineHlsFallbackIndex=-1;changedFormat=true;}}if(!changedFormat){boolean canRetrySame=!fromError&&tvInlineRecoveryCount<1;target=canRetrySame?current:(current+1<tvInlineSources.size()?current+1:current);}tvInlineRecoveryCount++;final int generation=tvPlaybackGeneration;'''
assert old in s
s=s.replace(old,new,1)
